...
<div class="sdg-paginator">
  {#if $resultInfo}
    ...
    <button
      class="sdg-paginator-nav-button"
      disabled={current <= 1}
      on:click={() => setPage(current - 1)}>Previous</button
    >
    {#if total > 5}
      ...
      {#each _window.pages as page}
        <button
          ...
          class="sdg-paginator-button">{page}</button
        >
      {/each}
      ...
      {/if}
      <button
        class:current={current === total}
        on:click={() => setPage(total)}
        class="sdg-paginator-button">{total}</button
      >
    {:else}
      {#each [...Array(total).keys()] as page}
        <button
          ...
          class="sdg-paginator-button">{page + 1}</button
        >
      {/each}
    {/if}
    <button
